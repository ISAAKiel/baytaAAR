...
NAs are allowed but neither rows nor columans may consist entirely of `NA` values. `bay.ta` will refuse to run in such cases, and the offending rows or columns need to be removed before analysis. Please see the `vignette("worked_example")` on Chelsea 'Old church' for an example of how this can be done.
