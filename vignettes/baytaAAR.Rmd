...
We will now run a first analysis with `bay.ta()` with the Sorsum data. We will leave most parameters at the default values and only change the minimum age to `18` and the thinning interval (`thinsteps`) to `100`. This number is multiplied with the number of saved steps (`10000` by default) and the result divided by the number of chains (default = 3) to obtain the total number of iterations, minus the number of iterations used for burning-in. We also set a seed for reproducibility.
...
From the minimum ESS value of around 40 (see above), it can be surmised that roughly 250 times more iterations are needed to get to a value of 10,000. This would increase runtime proportionally (approximately 200--250 minutes, equaling about 4 hours). However, because the resulting file would also be 250 times larger, some degree of thinning is necessary, so saving only, say, every 100th step (= thinning of 100). This in turn might increase autocorrelation which in turn will force you to run the model even longer.
...
To illustrate the spread of the ages, this time we rely on the functionality of the R package `tidybayes` [@kay_2024]. Its function `tidybayes::spread_draws()` allows to subset the chains in a single step to extract the estimated ages (`age.s`). We limit here the age estimates to the first seven.
...
sorsum_as_res |> tidybayes::spread_draws(age.s[age_number])  |> 
  ...
  labs( x = "\nModal age-at-death (years)", y = NULL ) + theme_light() +
...
The Gompertz function provides a different perspective on the mortality structure of the population studied as it does not depend on individual point estimates of ages like, for example, [Kaplan-Meier-diagrams](https://en.wikipedia.org/wiki/Kaplan–Meier_estimator).
...
At this point, we leave the further analysis following the same steps as above to the reader.
