...
chelsea_complete$known_age |> na.omit() |> as.numeric()
```
...
chelsea_complete |>
  select(starts_with("Dental"),
         starts_with("Pubic"),
         starts_with("Auricular"),
         starts_with("Costo")) |>
  pivot_longer(everything(), names_to = "method", values_to = "phase") |>
  mutate(phase = as.character(phase)) |>
  count(method, phase) |>
  pivot_wider(names_from = phase, values_from = n, values_fill = NA) |>
  knitr::kable ()
```
...
Please note that the models should run until the resulting Markov Chain Monte Carlo (MCMC) samples meet certain quality criteria. As already noted in the introductory article (`vignette("baytaAAR")`), these are `10,000` for `ESS` and below `1.1` for `PSRF`. To spare the time in running the models, we saved RDA-files with the raw MCMC-chains on `Github` for downloading and loading. If you really want to run the models, set `runNewMCMC` to `TRUE`.
...
if ( runNewMCMC ) {
  ...
  temp <- tempfile()
  download.file(paste0(path, file), destfile = temp)
  load(temp)
...
rownames(thresholds) <- c("dental_wear", "pubic_symph", "auricular_surf",
                          "costochondral", "ps_as - ps",  "ps_as - as",
                          "no NAs - dw", "no NAs - ps", "no NAs - as",
                          "no NAs - costo", "mnorm - dw", "mnorm - ps", 
                          "mnorm - as", "mnorm - costo")
